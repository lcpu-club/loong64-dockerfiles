...
if [ -z "$ports" ]; then
	standardMirrors=( 'http://mirrors.cloud.tencent.com/debian' )
	snapshotStandardMirrors=( "$("$thisDir/.snapshot-url.sh" "@$epoch")" )
else
	standardMirrors=( 'http://mirrors.cloud.tencent.com/debian-ports' )
...
if [ -n "$eol" ]; then
	...
	standardMirrors=( 'http://mirrors.cloud.tencent.com/debian' "${standardMirrors[@]}" )
	snapshotStandardMirrors=( "$archiveSnapshotMirror/debian" "${snapshotStandardMirrors[@]}" )

	securityMirrors=( 'http://mirrors.cloud.tencent.com/debian-security' "${securityMirrors[@]}" )
